class DonationsController < ApplicationController
  ...
  before_filter :administrator_required, :only => [:index]
  ...
  def new
    ...
    @donation = @project.donations.build(:user => self.current_user)
  ...
  def create
    @project = Project.find(params[:project])
    @donation = @project.donations.build(params[:donation])
    ...
    respond_to do |format|
      if @donation.save
        ...
        format.xml  { render :xml => @donation, :status => :created,
          :location => @donation }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @donation.errors,
